...
function playRecording(recording) {
  ...
  const bpm = parseInt(bpmInput.value);
  const milSecs = 60 / bpm * 1000;
  const totalDuration = recording.length * milSecs;

  let playTime = 0;

  recording.forEach((soundKey, index) => {
    const path = keySounds[soundKey];
    const sound = new Audio(path);
    soundsPlay.push(sound);
    ...
    playTime += milSecs; // Erhöhen Sie die Wartezeit für den nächsten Ton
  });

  setTimeout(() => {
    stopPlayback();
  }, totalDuration); // Stoppen Sie die Wiedergabe nach der Gesamtdauer des Recordings
}
